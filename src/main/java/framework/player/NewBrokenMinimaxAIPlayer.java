...
public abstract class NewBrokenMinimaxAIPlayer extends AIPlayer implements BoardObserver {
    ...
    private final Set<MoveOutcome> moveOutcomeSet = new HashSet<>();
    private final Set<MoveOutcome> doneMoveOutcomeSet = new HashSet<>();
...
//    private Move bestMove;
    ...
    public NewBrokenMinimaxAIPlayer(Board board, String name, AIDifficulty difficulty) {
        super(board, name);
    ...
    public NewBrokenMinimaxAIPlayer(Board board, AIDifficulty difficulty) {
        super(board);
...
//            bestMove = null;
        ...
        synchronized (moveOutcomeSet) {
            moveOutcomeSet.clear();
        }

        synchronized (doneMoveOutcomeSet) {
            doneMoveOutcomeSet.clear();
        }
        ...
        new Thread(() -> {
            ...
            Map<Move, AtomicInteger> winsMinusLosses = new HashMap<>();
            board.getValidMoves().forEach(boardPiece -> winsMinusLosses.put(new Move(boardPiece.getX(), boardPiece.getY()), new AtomicInteger(0)));

            synchronized (moveOutcomeSet) {
                moveOutcomeSet.forEach(outcome -> {
                    AtomicInteger value = winsMinusLosses.computeIfAbsent(outcome.firstMove, k -> new AtomicInteger(0));
                    if (outcome.boardValue > 0) {
                        value.addAndGet(highestDepth - outcome.depth + 1);
                    } else if (outcome.boardValue < 0) {
                        value.addAndGet(-(highestDepth - outcome.depth + 1));
                    }
                });
            }
            synchronized (doneMoveOutcomeSet) {
                doneMoveOutcomeSet.forEach(outcome -> {
                    AtomicInteger value = winsMinusLosses.computeIfAbsent(outcome.firstMove, k -> new AtomicInteger(0));
                    if (outcome.boardValue > 0) {
                        value.addAndGet(highestDepth - outcome.depth + 1);
                    } else if (outcome.boardValue < 0) {
            ...
            for(Move move : winsMinusLosses.keySet()) {
                int value = winsMinusLosses.get(move).get();
...
//            System.out.println("bestMove = " + bestMove);
    ...
    private boolean calculateNextDepth(UUID session) {
        ...
        synchronized (moveOutcomeSet) {
            moveOutcomesSet_copy = new HashSet<>(moveOutcomeSet);
        }
        ...
        for(MoveOutcome moveOutcome : moveOutcomesSet_copy) {
            ...
            synchronized (moveOutcomeSet) {
                moveOutcomeSet.remove(moveOutcome);
            }
...
//
        ...
        if(!outcome.done) {
            synchronized (moveOutcomeSet) {
                moveOutcomeSet.add(outcome);
            }
        }else{
            synchronized (doneMoveOutcomeSet) {
                doneMoveOutcomeSet.add(outcome);
            }
        }
    }
